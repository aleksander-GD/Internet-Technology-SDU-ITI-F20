...
<head>
    <title>PHP API test application</title>
    ...
    <div class="container">
        ...
        <form method="POST">
            ...
            <div class="form-group">
                ...
                <input type="text" name="projectname" id="projectname" class="form-text text-muted" value="<?= $_POST['projectname'] ?? 'aldus17' ?>" />
            </div>

            <div class="form-group">
                <label for="username">Username for registered user</label>
                <input type="text" name="username" id="username" class="form-text text-muted" value="<?= $_POST['username'] ?? 'willien' ?>" />
            </div>

            <div class="form-group">
                <label for="password">Password for registered user</label>
                <input type="text" name="password" id="password" class="form-text text-muted" value="<?= $_POST['password'] ?? 'test' ?>" />
            </div>
        ...
        if (isset($_POST['port'])) {
            ...
            if (isset($users[0]->username)) : ?>
                ...
                if ($foundName) : ?>
                    <p class="pass">
                        <i class="fas fa-check"></i> Userlist contains username <?= $_POST['username'] ?>
                    </p>
                <?php else : ?>
                    <p class="fail">
                        <i class="fas fa-times"></i> Userlist does not contain username <?= $_POST['username'] ?>
                    </p>
                <?php endif; ?>
            <?php else : ?>
                <p class="fail">
                    <i class="fas fa-times"></i> GET users failed<br />
                    - The username field should be named "username". An array with two usernames in JSON could look like this:<br />
            ...
            if (isset($pictures[0]->image)) : ?>
                ...
                if ($foundRightImage) : ?>
                    <p class="pass">
                        ...
                        <img src="<?= $pictures[$imageArrayId]->image ?>" alt="<?= $pictures[$imageArrayId]->title ?>" title="<?= $pictures[$imageArrayId]->description ?>" width="50px" />
                    </p>
                <?php else : ?>
                    <p class="fail">
                        <i class="fas fa-times"></i> Could not find the image that was posted <img src="<?= $pictures[0]->image ?>" alt="<?= $pictures[0]->title ?>" title="<?= $pictures[0]->description ?>" width="50px" />
                    </p>
                <?php endif; ?>
            <?php else : ?>
                <p class="fail">
                    <i class="fas fa-times"></i> GET pictures by user_id is failing. Expected format:<br />
            ...
            if (isset($pictures[0]->image)) : ?>
                ...
                <?php else : ?>
                    <p class="pass">
                        <i class="fas fa-check"></i> Could not find the image that was posted with wrong credentials
                    </p>
                <?php endif; ?>
        <?php endif;
        }
